...
for(current_participant in 1:length(list_all_system_log)){
  ...
  dat_current_participant <- dat_current_participant %>%
    mutate(ts_condition_check_completed_utc = as_datetime(x = as.numeric(V1)/1000, tz = "UTC"))
  ...
  dat_current_participant <- dat_current_participant %>%
    ...
    mutate(privacy_data_not_found = grepl(pattern = "[data not found]", .data[["extracted_raw_privacy"]], fixed = TRUE)) %>%
    mutate(privacy_activated = grepl(pattern = "privacy enabled", .data[["extracted_raw_privacy"]], fixed = TRUE)) %>%
    mutate(privacy_not_activated = grepl(pattern = "[privacy status=false]", .data[["extracted_raw_privacy"]], fixed = TRUE)) %>%
    mutate(driving_data_not_found = grepl(pattern = "[data not found]", .data[["extracted_raw_driving"]], fixed = TRUE)) %>%
  ...
  dat_current_participant <- dat_current_participant %>%
    select(mars_id, ts_condition_check_completed_mountain, olson,
           is_2qs, is_rand, is_ema, V6, evaluated_true,
...
saveRDS(dat_parsed_conditions, file = file.path(path_check_conditions_pipeline_data, "dat_parsed_conditions.rds"))
