...
# -----------------------------------------------------------------------------
# Extract rows associated with trigger time of 2QS and Random EMA
# -----------------------------------------------------------------------------
...
for(i in 1:length(list_all_system_log)){
  ...
  dat_ema <- dat_system_log %>%
    filter(grepl(pattern = "BackgroundService/Listen(init)/Scheduler(EMA-RANDOM)", .data[["V5"]], fixed = TRUE)) %>%
    filter(grepl(pattern = "try", .data[["V5"]], fixed = TRUE)) %>%
    filter(grepl(pattern = "trigger", .data[["V5"]], fixed = TRUE)) %>%
    filter(grepl(pattern = "success", .data[["V6"]], fixed = TRUE))
  
  list_2qs <- append(list_2qs, list(dat_2qs))
  list_ema <- append(list_ema, list(dat_ema))
}

# -----------------------------------------------------------------------------
# Parse block-level information from 2qs logs
# -----------------------------------------------------------------------------

list_dat_triggered <- list()

for(i in 1:length(list_2qs)){
  if(nrow(list_2qs[[i]]) > 0){
    dat_triggered <- list_2qs[[i]]
    ...
    # V1 -- contains the timestamp when the 2qs was triggered by the system
    ...
    dat_triggered <- dat_triggered %>%
      mutate(ts_2qs_utc = as_datetime(x = V1/1000, tz = "UTC"))  %>%
    ...
    dat_triggered[["ts_2qs_mountain"]] <- with_tz(time = dat_triggered[["ts_2qs_utc"]], tzone = "US/Mountain")
    
    # V5 -- contains the begin and end time of blocks;
    #       the displayed human-readable times are in the participant's LOCAL TIME
    #       this column also records the block in which the 2qs was triggered.
    ...
    dat_triggered <- dat_triggered %>% 
      ...
      mutate(block_number = case_when(
        grepl(pattern = "EMA-TAILOR-0", x = .data[["block_info"]]) ~ 0,
        grepl(pattern = "EMA-TAILOR-1", x = .data[["block_info"]]) ~ 1,
        grepl(pattern = "EMA-TAILOR-2", x = .data[["block_info"]]) ~ 2,
        grepl(pattern = "EMA-TAILOR-3", x = .data[["block_info"]]) ~ 3,
        grepl(pattern = "EMA-TAILOR-4", x = .data[["block_info"]]) ~ 4,
        grepl(pattern = "EMA-TAILOR-5", x = .data[["block_info"]]) ~ 5,
        .default = NULL
      ))
    ...
    dat_triggered[["block_bounds_mountain"]] <- block_mountain
    
    dat_triggered <- dat_triggered %>%
      select(mars_id, olson, block_number, 
             block_bounds_mountain, ts_2qs_mountain)
    
    list_dat_triggered <- append(list_dat_triggered, list(dat_triggered))
  }
}

list_dat_triggered_2qs <- list_dat_triggered

# -----------------------------------------------------------------------------
# Parse block-level information from Random EMA logs
# -----------------------------------------------------------------------------

list_dat_triggered <- list()

for(i in 1:length(list_ema)){
  if(nrow(list_ema[[i]]) > 0){
    dat_triggered <- list_ema[[i]]
    ...
    # V1 -- contains the timestamp when the ema was triggered by the system
    # V3 -- displayed human-readable times are in the participant's LOCAL TIME;
    #       a good sanity check to see if you are converting V1 to 
    #       human-readable time correctly is to match what you get to V3
    dat_triggered <- dat_triggered %>%
      mutate(ts_ema_utc = as_datetime(x = V1/1000, tz = "UTC"))  %>%
      mutate(olson = case_when(
        V2/(1000*60*60) == -5 ~ "US/Eastern", 
        V2/(1000*60*60) == -6 ~ "US/Central", 
        V2/(1000*60*60) == -7 ~ "US/Mountain", 
    ...
    # even for participants not actually in mountain time, 
    # so that we have a quick way to eye-ball the data when we do quick checks.
    dat_triggered[["ts_ema_mountain"]] <- with_tz(time = dat_triggered[["ts_ema_utc"]], tzone = "US/Mountain")
    
    # V5 -- contains the begin and end time of blocks;
    #       the displayed human-readable times are in the participant's LOCAL TIME
    #       this column also records the block in which the ema was triggered.
    ...
    dat_triggered <- dat_triggered %>% 
      mutate(block_info = block_info) %>%
      mutate(block_number = case_when(
        grepl(pattern = "EMA-RANDOM-0", x = .data[["block_info"]]) ~ 0,
        grepl(pattern = "EMA-RANDOM-1", x = .data[["block_info"]]) ~ 1,
        grepl(pattern = "EMA-RANDOM-2", x = .data[["block_info"]]) ~ 2,
        grepl(pattern = "EMA-RANDOM-3", x = .data[["block_info"]]) ~ 3,
        grepl(pattern = "EMA-RANDOM-4", x = .data[["block_info"]]) ~ 4,
        grepl(pattern = "EMA-RANDOM-5", x = .data[["block_info"]]) ~ 5,
        .default = NULL
      ))
    ...
    # block happened.
    #
    # While the code logic to obtain delivery_bounds_mountain for Random EMA
    # is the same code logic to obtain block_bounds_mountain for 2QS
    # the interpretation of the resulting value differs.
    # For Random EMA, the time interval pertains to the span of time the software 
    # will try to deliver the Random EMA for that block.
    block_mountain <- bind_rows(lapply(list_interval_converted, FUN = as.data.frame))
    block_mountain <- block_mountain$`X[[i]]`
    dat_triggered[["delivery_bounds_mountain"]] <- block_mountain
    
    dat_triggered <- dat_triggered %>%
      select(mars_id, olson, block_number, 
             delivery_bounds_mountain, ts_ema_mountain)
    
    list_dat_triggered <- append(list_dat_triggered, list(dat_triggered))
  }
}

list_dat_triggered_ema <- list_dat_triggered
...
count_events_within_list <- function(x){
  ...
  is_col_here <- ("delivery_bounds_mountain" %in% colnames(x))
  
  if(is_col_here){
    curr_id <- x[["mars_id"]][1]
    out <- x %>% 
      group_by(delivery_bounds_mountain) %>% 
    ...
    curr_id <- x[["mars_id"]][1]
    out <- x %>% 
      group_by(block_bounds_mountain) %>% 
      summarise(b = min(block_number), count = n()) %>%
      mutate(mars_id = curr_id) %>%
      select(mars_id, everything())
  }
...
table_2qs <- lapply(list_dat_triggered_2qs, FUN = count_events_within_list)
table_2qs <- bind_rows(table_2qs)
#print(table_2qs)  # Uncomment to reveal table

table_ema <- lapply(list_dat_triggered_ema, FUN = count_events_within_list)
table_ema <- bind_rows(table_ema)
#print(table_ema)  # Uncomment to reveal table
...
list_dat_triggered_2qs <- lapply(list_dat_triggered_2qs, FUN = dedup_within_list)
list_dat_triggered_ema <- lapply(list_dat_triggered_ema, FUN = dedup_within_list)

# -----------------------------------------------------------------------------
# Rename columns to be more descriptive
# -----------------------------------------------------------------------------

list_dat_triggered_2qs <- lapply(list_dat_triggered_2qs, function(dat_participant){
  dat_participant <- dat_participant %>% rename(ts_2qs_triggered_mountain = ts_2qs_mountain)
  return(dat_participant)
})

list_dat_triggered_ema <- lapply(list_dat_triggered_ema, function(dat_participant){
  dat_participant <- dat_participant %>% rename(ts_ema_triggered_mountain = ts_ema_mountain)
  return(dat_participant)
})
...
saveRDS(list_dat_triggered_2qs, file = file.path(path_manipulated_data, "list_dat_triggered_2qs.rds"))
saveRDS(list_dat_triggered_ema, file = file.path(path_manipulated_data, "list_dat_triggered_ema.rds"))
