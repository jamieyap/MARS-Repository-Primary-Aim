...
dat_cc_unadjusted <- dat_for_analysis %>% 
  select(participant_id, total_engagement_combined, C_V4_7dayPP) %>% 
  filter(complete.cases(.))
```

```{r}
dat_tmp_summary_engagement_unadjusted <- dat_cc_unadjusted %>%
  select(C_V4_7dayPP, total_engagement_combined) %>%
  filter(complete.cases(.)) %>%
...
```{r}
cor.test(dat_cc_unadjusted[["total_engagement_combined"]], dat_cc_unadjusted[["C_V4_7dayPP"]])
```

```{r}
fit <- geeglm(C_V4_7dayPP ~ total_engagement_combined, 
              family = "poisson", 
...
```{r}
fit <- geeglm(C_V4_7dayPP ~ I(total_engagement_combined >= 6), 
              family = "poisson", 
              data = dat_cc, 
              id = participant_id, 
              waves = NULL)
...
```{r}
fit <- geeglm(C_V4_7dayPP ~ I(total_engagement_combined >= 12), 
              family = "poisson", 
              data = dat_cc, 
              id = participant_id, 
              waves = NULL)
...
```{r}
fit <- geeglm(C_V4_7dayPP ~ total_engagement_combined + c_age + c_is_male + c_is_latino + c_is_not_latino_and_black + c_is_not_latino_and_other + c_baseline_tobacco_history + c_has_partner + c_income_val, 
              family = "poisson", 
              data = dat_cc, 
              id = participant_id, 
              waves = NULL)
...
```{r}
fit <- geeglm(C_V4_7dayPP ~ I(total_engagement_combined >= 12) + c_age + c_is_male + c_is_latino + c_is_not_latino_and_black + c_is_not_latino_and_other + c_baseline_tobacco_history + c_has_partner + c_income_val, 
              family = "poisson", 
              data = dat_cc, 
              id = participant_id, 
              waves = NULL)
