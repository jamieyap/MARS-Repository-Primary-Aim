...
logical_vec_cc <- dat_for_analysis %>% 
  select(Y, substance_is_any,
         substance_is_any_nicotine_lead1) %>%
  complete.cases(.)
...
dat_summary <- data.frame(n_elig_overall_next_dp = sum(dat_summary_observed[["total"]]),
                          n_included_among_elig_overall_next_dp = dat_summary_observed[dat_summary_observed[["is_complete"]] == TRUE, ][["total"]],
                          n_excluded_among_elig_overall_next_dp = dat_summary_observed[dat_summary_observed[["is_complete"]] == FALSE, ][["total"]])
