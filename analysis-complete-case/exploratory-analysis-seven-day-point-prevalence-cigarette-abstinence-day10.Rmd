---
title: "Seven-Day Cigarette Point Prevalence Abstinence on Day 10"
...
is_dir_exist <- file.exists(file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Seven-Day-Point-Prevalence-Cigarette-Abstinence-Day10"))

if(isFALSE(is_dir_exist)){
  dir.create(file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Seven-Day-Point-Prevalence-Cigarette-Abstinence-Day10"))
}
```
...
dat_for_analysis2 <- dat_primary_aim %>%
  mutate(engagement_combined = case_when(
    eligibility == 0 ~ 0,
    (eligibility == 1) & (!is.na(Y)) ~ Y,
    (eligibility == 1) & (is.na(Y)) ~ 0,
  ...
  filter(is_include_main ==1) %>%
  group_by(mars_id, participant_id, cluster_id) %>%
  summarise(total_engagement_combined = sum(engagement_combined),
            total_eligible_dp = sum(eligibility)) %>%
  ungroup(.) %>% 
  group_by(mars_id) %>% 
  summarise(total_days_with_engagement = sum(total_engagement_combined > 0))%>%
  ungroup(.) 

dat_for_analysis <- left_join(x = dat_for_analysis, y = dat_for_analysis2, by = join_by(mars_id == mars_id))
```

```{r}
saveRDS(dat_for_analysis, file.path(path_manipulated_data, "dat_for_analysis_7dayPP.rds"))
```

# Analyses with `C_V3_7dayPP` as the outcome

```{r}
dat_cc_unadjusted <- dat_for_analysis %>% 
  select(participant_id, total_engagement_combined, C_V3_7dayPP) %>% 
  filter(complete.cases(.))
```

```{r}
dat_tmp_summary_engagement_unadjusted <- dat_cc_unadjusted %>%
  select(C_V3_7dayPP, total_engagement_combined) %>%
  filter(complete.cases(.)) %>%
...
dat_tmp_summary_engagement_unadjusted
```

```{r}
dat_cc <- dat_for_analysis %>% 
  select(participant_id, total_engagement_combined, C_V3_7dayPP, all_of(these_control_covariates)) %>% 
  filter(complete.cases(.))
```

```{r}
dat_tmp_summary_engagement <- dat_cc %>%
  select(C_V3_7dayPP, total_engagement_combined, all_of(these_control_covariates)) %>%
  filter(complete.cases(.)) %>%
  summarise(n_participants = n(), 
            q25_engaged = quantile(total_engagement_combined, p = 0.25),
            q33_engaged = quantile(total_engagement_combined, p = 0.33),
...
```{r}
dat_cc_V3_and_V4 <- dat_for_analysis %>% 
  select(participant_id, C_V3_7dayPP, C_V4_7dayPP) %>% 
  filter(complete.cases(.))

dat_cc_V3_and_V4 %>%
  select(C_V3_7dayPP, C_V4_7dayPP) %>%
  filter(complete.cases(.)) %>%
  summarise(n_participants = n())

cor.test(dat_cc_V3_and_V4[["C_V3_7dayPP"]], dat_cc_V3_and_V4[["C_V4_7dayPP"]])
```


## Log-Linear Model

### Unadjusted; Model with `total_engagement_combined` as a predictor

```{r}
cor.test(dat_cc_unadjusted[["total_engagement_combined"]], dat_cc_unadjusted[["C_V3_7dayPP"]])
```

```{r}
fit <- geeglm(C_V3_7dayPP ~ total_engagement_combined, 
              family = "poisson", 
              data = dat_cc_unadjusted, 
              id = participant_id, 
              waves = NULL)
...
dat_result_prob_scale <- data.frame(est_contrast_exp = exp(dat_result_log_scale[["est_contrast"]]),
                                    conf_interval_LB_exp = exp(dat_result_log_scale[["conf_interval_LB"]]),
                                    conf_interval_UB_exp = exp(dat_result_log_scale[["conf_interval_UB"]]))

row.names(dat_result_prob_scale) <- row.names(dat_result_log_scale)

# Format output to 3 decimal places
dat_result_prob_scale %>% round(., 3)
```

### Unadjusted; Model with `I(total_engagement_combined >= 6)` as a predictor

```{r}
fit <- geeglm(C_V3_7dayPP ~ I(total_engagement_combined >= 6), 
              family = "poisson", 
              data = dat_cc_unadjusted, 
              id = participant_id, 
              waves = NULL)
...
# Format output to 3 decimal places
dat_result_probability_scale %>% round(., 3)
```

```{r}
Lmat <- matrix(c(1, 0,   # Low
                 1, 1),  # Not low
               ncol = 2, byrow = TRUE)
est_contrast <- Lmat %*% as.matrix(fit$coefficients)
est_vcov_contrast <- Lmat %*% vcov(fit) %*% t(Lmat)
...
### Unadjusted; Model with `I(total_engagement_combined >= 12)` as a predictor

```{r}
fit <- geeglm(C_V3_7dayPP ~ I(total_engagement_combined >= 12), 
              family = "poisson", 
              data = dat_cc_unadjusted, 
              id = participant_id, 
              waves = NULL)
...
### Adjusted; Model with `total_engagement_combined` as a predictor

```{r}
fit <- geeglm(C_V3_7dayPP ~ total_engagement_combined + c_age + c_is_male + c_is_latino + c_is_not_latino_and_black + c_is_not_latino_and_other + c_baseline_tobacco_history + c_has_partner + c_income_val, 
              family = "poisson", 
              data = dat_cc, 
              id = participant_id, 
              waves = NULL)
...
### Adjusted; Model with `I(total_engagement_combined >= 6)` as a predictor

```{r}
fit <- geeglm(C_V3_7dayPP ~ I(total_engagement_combined >= 6) + c_age + c_is_male + c_is_latino + c_is_not_latino_and_black + c_is_not_latino_and_other + c_baseline_tobacco_history + c_has_partner + c_income_val, 
              family = "poisson", 
              data = dat_cc, 
              id = participant_id, 
              waves = NULL)
...
# Format output to 3 decimal places
dat_result_log_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Seven-Day-Point-Prevalence-Cigarette-Abstinence-Day10", "model_params_log_scale.csv"), row.names = TRUE)
```
...
# Format output to 3 decimal places
dat_result_probability_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Seven-Day-Point-Prevalence-Cigarette-Abstinence-Day10", "model_params_probability_scale.csv"), row.names = TRUE)
```
...
# Format output to 3 decimal places
dat_result_log_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Seven-Day-Point-Prevalence-Cigarette-Abstinence-Day10", "simple_effects_log_scale.csv"), row.names = TRUE)
...
# Format output to 3 decimal places
dat_result_prob_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Seven-Day-Point-Prevalence-Cigarette-Abstinence-Day10", "simple_effects_probability_scale.csv"), row.names = TRUE)
```

### Adjusted; Model with `I(total_engagement_combined >= 12)` as a predictor

```{r}
fit <- geeglm(C_V3_7dayPP ~ I(total_engagement_combined >= 12) + c_age + c_is_male + c_is_latino + c_is_not_latino_and_black + c_is_not_latino_and_other + c_baseline_tobacco_history + c_has_partner + c_income_val, 
              family = "poisson", 
              data = dat_cc, 
              id = participant_id, 
              waves = NULL)
...
dat_result_odds_scale <- data.frame(est_contrast_exp = exp(dat_result_log_scale[["est_contrast"]]),
                                    conf_interval_LB_exp = exp(dat_result_log_scale[["conf_interval_LB"]]),
                                    conf_interval_UB_exp = exp(dat_result_log_scale[["conf_interval_UB"]]))

row.names(dat_result_odds_scale) <- row.names(dat_result_log_scale)

# Format output to 3 decimal places
dat_result_odds_scale %>% round(., 3)
```
