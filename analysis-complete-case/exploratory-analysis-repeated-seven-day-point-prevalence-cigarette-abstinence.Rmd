---
title: "Repeated Seven-Day Cigarette Point Prevalence Abstinence"
...
is_dir_exist <- file.exists(file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Repeated-Seven-Day-Point-Prevalence-Cigarette-Abstinence"))

if(isFALSE(is_dir_exist)){
  dir.create(file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Repeated-Seven-Day-Point-Prevalence-Cigarette-Abstinence"))
}
```
...
dat_for_analysis <- dat_for_analysis %>%
  mutate(C_repeated_7dayPP = case_when(
    (C_V3_7dayPP == 1) & (C_V4_7dayPP == 1) ~ 1,
    (C_V3_7dayPP != 1) | (C_V4_7dayPP != 1) ~ 0,
    .default = NULL
  )) %>%
  mutate(C_repeated_7dayPP = replace(C_repeated_7dayPP, (!is.na(C_V3_7dayPP) & is.na(C_V4_7dayPP)) & (C_V3_7dayPP == 0), 0)) %>%
  mutate(C_repeated_7dayPP = replace(C_repeated_7dayPP, (is.na(C_V3_7dayPP) & !is.na(C_V4_7dayPP)) & (C_V4_7dayPP == 0), 0)) %>%
  mutate(C_repeated_7dayPP_nomiss = C_repeated_7dayPP) %>%
  mutate(C_repeated_7dayPP_nomiss = replace(C_repeated_7dayPP_nomiss, is.na(C_repeated_7dayPP), 0))
```

```{r}
dat_for_analysis %>%
  group_by(C_V3_7dayPP, C_V4_7dayPP, C_repeated_7dayPP) %>%
  summarise(n())
```
...
```{r}
dat_for_analysis %>%
  group_by(C_repeated_7dayPP, C_repeated_7dayPP_nomiss) %>%
  summarise(n())
```

```{r}
dat_tmp_summary_engagement <- dat_for_analysis %>%
  select(C_repeated_7dayPP, total_engagement_combined, all_of(these_control_covariates)) %>%
  filter(complete.cases(.)) %>%
  summarise(q25_engaged = quantile(total_engagement_combined, p = 0.25),
...
# Analyses with `C_repeated_7dayPP` (Repeated Seven-Day Cigarette Point Prevalence Abstinence) as the outcome

```{r}
dat_cc_unadjusted <- dat_for_analysis %>% 
  select(participant_id, total_engagement_combined, C_repeated_7dayPP) %>% 
  filter(complete.cases(.))
```

```{r}
dat_tmp_summary_engagement_unadjusted <- dat_cc_unadjusted %>%
  select(C_repeated_7dayPP, total_engagement_combined) %>%
  filter(complete.cases(.)) %>%
...
dat_cc <- dat_for_analysis %>% 
  select(participant_id, total_engagement_combined, C_repeated_7dayPP, all_of(these_control_covariates)) %>% 
  filter(complete.cases(.))
```
...
```{r}
cor.test(dat_cc_unadjusted[["total_engagement_combined"]], dat_cc_unadjusted[["C_repeated_7dayPP"]])
```

```{r}
fit <- geeglm(C_repeated_7dayPP ~ total_engagement_combined, 
              family = "poisson", 
...
```{r}
fit <- geeglm(C_repeated_7dayPP ~ total_engagement_combined + c_age + c_is_male + c_is_latino + c_is_not_latino_and_black + c_is_not_latino_and_other + c_baseline_tobacco_history + c_has_partner + c_income_val, 
              family = "poisson", 
              data = dat_cc, 
              id = participant_id, 
              waves = NULL)
...
```{r}
fit <- geeglm(C_repeated_7dayPP ~ I(total_engagement_combined >= 6) + c_age + c_is_male + c_is_latino + c_is_not_latino_and_black + c_is_not_latino_and_other + c_baseline_tobacco_history + c_has_partner + c_income_val, 
              family = "poisson", 
              data = dat_cc, 
              id = participant_id, 
              waves = NULL)
...
# Format output to 3 decimal places
dat_result_log_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Repeated-Seven-Day-Point-Prevalence-Cigarette-Abstinence", "model_params_log_scale.csv"), row.names = TRUE)
```
...
# Format output to 3 decimal places
dat_result_probability_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Repeated-Seven-Day-Point-Prevalence-Cigarette-Abstinence", "model_params_probability_scale.csv"), row.names = TRUE)
```
...
# Format output to 3 decimal places
dat_result_log_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Repeated-Seven-Day-Point-Prevalence-Cigarette-Abstinence", "simple_effects_log_scale.csv"), row.names = TRUE)
...
# Format output to 3 decimal places
dat_result_prob_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Repeated-Seven-Day-Point-Prevalence-Cigarette-Abstinence", "simple_effects_probability_scale.csv"), row.names = TRUE)
```
