...
dat_for_analysis <- dat_primary_aim %>%
  select(participant_id, cluster_id, decision_point, days_between_v1_and_coinflip_local,
         Y, substance_is_cigarettes, cigarette_counts,
  ...
  mutate(substance_is_cigarettes_lead1 = lead(substance_is_cigarettes),
         cigarette_counts_lead1 = lead(cigarette_counts),
...
```{r}
dat_for_analysis %>%
  group_by(is.na(substance_is_cigarettes), is.na(cigarette_counts)) %>%
  summarise(count = n())
```
...
dat_logreg <- dat_for_analysis %>% 
  ...
  select(participant_id, decision_point, substance_is_cigarettes, cigarette_counts, Y,
         coinflip, any_prompt,
         age, is_male, is_latino, is_not_latino_and_black, is_not_latino_and_other, 
         baseline_tobacco_history, has_partner, income_val) %>%
...
```{r}
fit <- geeglm(cigarette_counts ~ c_age + c_is_male + c_is_latino + c_is_not_latino_and_black + c_is_not_latino_and_other + c_baseline_tobacco_history + c_has_partner + c_income_val + c_any_prompt + Y, family = "gaussian", data = dat_logreg, id = participant_id, waves = decision_point)
```
...
# Format output to 3 decimal places
dat_results_logodds_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Associational-Cigarette-Use", "block_level_mean_scale_concurrent.csv"), row.names = TRUE)
```

```{r}
fit <- geeglm(cigarette_counts ~ c_age + c_is_male + c_is_latino + c_is_not_latino_and_black + c_is_not_latino_and_other + c_baseline_tobacco_history + c_has_partner + c_income_val + c_any_prompt + Y, family = "gaussian", data = dat_logreg %>% filter(cigarette_counts > 0), id = participant_id, waves = decision_point)
```
...
# Format output to 3 decimal places
dat_results_logodds_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Associational-Cigarette-Use", "block_level_mean_scale_concurrent_among_smokers.csv"), row.names = TRUE)
```
...
dat_logreg <- dat_for_analysis %>% 
  ...
  select(participant_id, decision_point, substance_is_cigarettes_lead1, substance_is_cigarettes, cigarette_counts_lead1, cigarette_counts, Y,
         coinflip, any_prompt,
         age, is_male, is_latino, is_not_latino_and_black, is_not_latino_and_other, 
         baseline_tobacco_history, has_partner, income_val) %>%
  mutate(drop_row = if_else(!complete.cases(.), 1, 0)) %>%
...
```{r}
dat_tmp2 <- dat_logreg %>% 
  select(participant_id, decision_point, substance_is_cigarettes, cigarette_counts, any_prompt)
```
...
dat_tmp2_summary <- dat_tmp2 %>%
  ...
  summarise(m_any_prompt = mean(any_prompt),
            ...
            m_cigarette_counts = mean(cigarette_counts))
```
...
dat_logreg <- dat_logreg %>%
  ...
  mutate(c_substance_is_cigarettes = substance_is_cigarettes - m_substance_is_cigarettes,
         c_cigarette_counts = cigarette_counts - m_cigarette_counts)
```
...
```{r}
fit <- geeglm(cigarette_counts_lead1 ~ c_age + c_is_male + c_is_latino + c_is_not_latino_and_black + c_is_not_latino_and_other + c_baseline_tobacco_history + c_has_partner + c_income_val + c_any_prompt + c_cigarette_counts + Y, family = "gaussian", data = dat_logreg, id = participant_id, waves = decision_point)
```
...
# Format output to 3 decimal places
dat_results_logodds_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Associational-Cigarette-Use", "block_level_mean_scale_lagged.csv"), row.names = TRUE)
```

```{r}
fit <- geeglm(cigarette_counts_lead1 ~ c_age + c_is_male + c_is_latino + c_is_not_latino_and_black + c_is_not_latino_and_other + c_baseline_tobacco_history + c_has_partner + c_income_val + c_any_prompt + c_cigarette_counts + Y, family = "gaussian", data = dat_logreg %>% filter(cigarette_counts_lead1 > 0), id = participant_id, waves = decision_point)
```
...
# Format output to 3 decimal places
dat_results_logodds_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Associational-Cigarette-Use", "block_level_mean_scale_lagged_among_smokers.csv"), row.names = TRUE)
```
