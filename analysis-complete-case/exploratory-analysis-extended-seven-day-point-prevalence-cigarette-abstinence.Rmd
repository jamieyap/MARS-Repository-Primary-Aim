---
title: "Extended Seven-Day Cigarette Point Prevalence Abstinence"
...
is_dir_exist <- file.exists(file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Extended-Seven-Day-Point-Prevalence-Cigarette-Abstinence"))

if(isFALSE(is_dir_exist)){
  dir.create(file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Extended-Seven-Day-Point-Prevalence-Cigarette-Abstinence"))
}
```
...
dat_for_analysis <- dat_for_analysis %>%
  mutate(sustained_cig_abstinence = case_when(
    (C_V3_7dayPP == 1) & (C_V4_7dayPP == 1) ~ 1,
    (C_V3_7dayPP != 1) | (C_V4_7dayPP != 1) ~ 0,
    .default = NULL
  )) %>%
  mutate(sustained_cig_abstinence_nomiss = sustained_cig_abstinence) %>%
  mutate(sustained_cig_abstinence_nomiss = replace(sustained_cig_abstinence_nomiss, is.na(sustained_cig_abstinence), 0))
```

```{r}
dat_for_analysis %>%
  group_by(sustained_cig_abstinence, sustained_cig_abstinence_nomiss) %>%
  summarise(n())
```

```{r}
dat_tmp_summary_engagement <- dat_for_analysis %>%
  select(sustained_cig_abstinence, total_engagement_combined, all_of(these_control_covariates)) %>%
...
# Analyses with `sustained_cig_abstinence` (Extended Seven-Day Cigarette Point Prevalence Abstinence) as the outcome

```{r}
dat_cc <- dat_for_analysis %>% 
  select(participant_id, total_engagement_combined, sustained_cig_abstinence, all_of(these_control_covariates)) %>% 
  filter(complete.cases(.))
```

```{r}
dat_tmp_summary_engagement <- dat_cc %>%
  select(sustained_cig_abstinence, total_engagement_combined) %>%
  filter(complete.cases(.)) %>%
...
```{r}
fit <- geeglm(sustained_cig_abstinence ~ I(total_engagement_combined >= 6) + c_age + c_is_male + c_is_latino + c_is_not_latino_and_black + c_is_not_latino_and_other + c_baseline_tobacco_history + c_has_partner + c_income_val, 
              family = "poisson", 
              data = dat_cc, 
              id = participant_id, 
              waves = NULL)
...
dat_result_log_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Extended-Seven-Day-Point-Prevalence-Cigarette-Abstinence", "model_params_log_scale.csv"), row.names = TRUE)
```
...
# Format output to 3 decimal places
dat_result_probability_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Extended-Seven-Day-Point-Prevalence-Cigarette-Abstinence", "model_params_probability_scale.csv"), row.names = TRUE)
```
...
# Format output to 3 decimal places
dat_result_log_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Extended-Seven-Day-Point-Prevalence-Cigarette-Abstinence", "simple_effects_log_scale.csv"), row.names = TRUE)
...
# Format output to 3 decimal places
dat_result_prob_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Extended-Seven-Day-Point-Prevalence-Cigarette-Abstinence", "simple_effects_probability_scale.csv"), row.names = TRUE)
```
