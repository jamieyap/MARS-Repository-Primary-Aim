...
# Marginal association between engagement in self-regulatory strategies and tobacco use on current day
...
plot(x_grid, prevalence, ylim = c(0,1), type = "l", xlab = "No. of times engaged in self-regulatory strategies on day d", ylab = "Prevalance of tobacco use on day d")
...
$$
\text{logit} Pr\left(\text{ANY TOBACCO USE}_{i} \left(t_{i,j} \right) = 1\right)= \beta_0 \left(t_{i,j}\right) + \beta_1 \left(t_{i,j}\right) \text{NO. OF TIMES ENGAGED} \left(t_{i,j}\right)
$$
...
# Marginal association between engagement in self-regulatory strategies and tobacco use on next day
...
prevalence_upper <- exp_upper/(1 + exp_upper)
prevalence_lower <- exp_lower/(1 + exp_lower)
plot(x_grid, prevalence, ylim = c(0,1), type = "l", xlab = "No. of times engaged in self-regulatory strategies on day d", ylab = "Prevalance of tobacco use on day d+1")
lines(x_grid, prevalence_upper, type = "l", lty = 2)
lines(x_grid, prevalence_lower, type = "l", lty = 2)
```
...
$$
\text{logit} Pr\left(\text{ANY TOBACCO USE}_{i} \left(t_{i,j+1} \right) = 1\right)= \beta_0 \left(t_{i,j}\right) + \beta_1 \left(t_{i,j}\right) \text{NO. OF TIMES ENGAGED} \left(t_{i,j}\right) + \beta_2 \left(t_{i,j}\right) \text{ANY TOBACCO USE}_{i} \left(t_{i,j}\right)
$$
