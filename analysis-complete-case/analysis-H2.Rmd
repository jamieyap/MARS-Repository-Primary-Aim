...
dat_prevalence <- dat_for_analysis %>%
  ...
  summarise(n_dp = n(),
            n_not_missing = sum(!is.na(Y)),
            mu_Y = mean(Y, na.rm = TRUE),
            num_engaged = sum(Y == 1, na.rm = TRUE),
            num_not_engaged = sum(Y == 0, na.rm = TRUE)) %>%
  arrange(desc(is_high_effort), desc(is_low_effort))

dat_prevalence <- as.data.frame(dat_prevalence)
...
row.names(dat_exp_scale) <- c("High vs. None", "Low vs. None", "High vs. Low")
...
```{r}
fit <- geeglm(Y ~ is_high_effort + is_low_effort + age + is_male + is_latino + is_not_latino_and_black + is_not_latino_and_other + baseline_tobacco_history + has_partner + income_val + hour_coinflip_local + days_between_v1_and_coinflip_local + any_response_2qs + any_recent_eligible_dp + engagement_most_recent_eligible, family = "gaussian", data = dat_for_analysis_elig, id = participant_id, waves = decision_point)
...
results_contrast2 <- data.frame(est_contrast = est_contrast, lcl_contrast = lcl_contrast, ucl_contrast = ucl_contrast, se_contrast = se_contrast, test_stat_contrast = test_stat_contrast, degrees_of_freedom = degrees_of_freedom, p_value = p_value_contrast)
row.names(results_contrast2) <- paste("High vs. Low at ", 1:8)
```

```{r}
# Format output to 3 decimal places
results_contrast_all <- rbind(results_contrast, results_contrast2)

results_contrast_all %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H2", "log_risk_ratio_scale_H2_linear_day_contrast.csv"), row.names = TRUE)
```

```{r, fig.width = 8, fig.height = 12}
dat_result_for_plotting <- results_contrast2

dat_result_for_plotting$days_since_start <- 1:8 
...
# call the functions convert_to_emee_fit_object and summary to facilitate this calculation
fit1_converted <- convert_to_emee_fit_object(fit1, dat_for_analysis, "participant_id")
fit1_results <- summary(fit1_converted, show_control_fit = TRUE, conf_level = 0.90)
fit1_causal <- fit1_results$causal_excursion_effect
fit1_control <- fit1_results$control_variables
```
...
dat_causal <- data.frame(coefficient = c("beta0", "beta1","beta2", "beta3"),
                          estimate = round(fit1_causal[,"Estimate"], 3),
                          LCL90 = round(fit1_causal[,"90% LCL"], 3),
                          UCL90 = round(fit1_causal[,"90% UCL"], 3),
                          SE = round(fit1_causal[,"StdErr"], 3),
                          test_stat = round(fit1_causal[,"t_value"], 3),
                          df = round(fit1_causal[, "df"], 3),
                          p = round(fit1_causal[,"p-value"], 3))
...
dat_control <- data.frame(coefficient = paste("alpha", 0:(length(fit1_control[,"Estimate"])-1), sep=""),
                          estimate = round(fit1_control[,"Estimate"], 3),
                          LCL90 = round(fit1_control[,"90% LCL"], 3),
                          UCL90 = round(fit1_control[,"90% UCL"], 3),
                          SE = round(fit1_control[,"StdErr"], 3),
                          test_stat = round(fit1_control[,"t_value"], 3),
                          df = round(fit1_control[,"df"], 3),
                          p = round(fit1_control[,"p-value"], 3))
...
# Format output to 3 decimal places
dat_result %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H2", "log_risk_ratio_scale_H2_linear_day_CI90.csv"), row.names = TRUE)
```
...
results_contrast_all %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H2", "log_risk_ratio_scale_H2_linear_day_contrast_CI90.csv"), row.names = TRUE)
```
...
results_contrast2 <- data.frame(est_contrast = est_contrast, lcl_contrast = lcl_contrast, ucl_contrast = ucl_contrast, se_contrast = se_contrast, test_stat_contrast = test_stat_contrast, degrees_of_freedom = degrees_of_freedom, p_value = p_value_contrast)
row.names(results_contrast2) <- paste("High vs. Low at ", 0:24)
```
...
```{r}
# Format output to 3 decimal places
results_contrast_all %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H2", "log_risk_ratio_scale_H2_linear_hour_contrast.csv"), row.names = TRUE)
```
...
# Format output to 3 decimal places
dat_result %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H2", "log_risk_ratio_scale_H2_linear_hour_CI90.csv"), row.names = TRUE)
```
...
```{r}
# Format output to 3 decimal places
results_contrast_all %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H2", "log_risk_ratio_scale_H2_linear_hour_contrast_CI90.csv"), row.names = TRUE)
```
...
# Format output to 3 decimal places
results_contrast_all %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H2", "log_risk_ratio_scale_H2_quadratic_day_contrast.csv"), row.names = TRUE)
```
...
# Format output to 3 decimal places
dat_result %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H2", "log_risk_ratio_scale_H2_quadratic_day_CI90.csv"), row.names = TRUE)
```
...
# Format output to 3 decimal places
results_contrast_all %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H2", "log_risk_ratio_scale_H2_quadratic_day_contrast_CI90.csv"), row.names = TRUE)
```
...
# Format output to 3 decimal places
results_contrast_all %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H2", "log_risk_ratio_scale_H2_quadratic_hour_contrast.csv"), row.names = TRUE)
```
...
# Format output to 3 decimal places
dat_result %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H2", "log_risk_ratio_scale_H2_quadratic_hour_CI90.csv"), row.names = TRUE)
```
...
# Format output to 3 decimal places
results_contrast_all %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H2", "log_risk_ratio_scale_H2_quadratic_hour_contrast_CI90.csv"), row.names = TRUE)
```
...
dat_result %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H2", "sensitivity_risk_difference_scale_H2.csv"), row.names = TRUE)
```
...
dat_result %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H2", "sensitivity_risk_difference_scale_contrast_H2.csv"), row.names = TRUE)
```
