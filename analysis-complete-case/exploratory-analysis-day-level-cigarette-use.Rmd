...
## Use model with only number of times engaged as predictor
...
## Use model with number of itmes engaged and control covariates as predictor

```{r}
fit <- geeglm(any_nic ~ n_engaged + age + is_male + is_latino + is_not_latino_and_black + is_not_latino_and_other + baseline_tobacco_history + has_partner + income_val, 
              family = "binomial", 
              data = dat_logreg[complete.cases(dat_logreg[, c("any_nic", "n_engaged", "age", "is_male", "is_latino", "is_not_latino_and_black", "is_not_latino_and_other", "baseline_tobacco_history", "has_partner", "income_val")]),], 
              id = participant_id, 
              waves = cluster_id)
```
...
plot(x_grid, prevalence, ylim = c(0,1), type = "l", xlab = "No. of times engaged in self-regulatory strategies on day d", ylab = "Prevalance of cigarette use on day d")
...
$$
\text{logit} Pr\left(\text{ANY CIGARETTE USE}_{i} \left(t_{i,j+1} \right) = 1\right)= \beta_0 \left(t_{i,j}\right) + \beta_1 \left(t_{i,j}\right) \text{NO. OF TIMES ENGAGED} \left(t_{i,j}\right) + \beta_2 \left(t_{i,j}\right) \text{ANY CIGARETTE USE}_{i} \left(t_{i,j}\right)
$$
