...
plot(dat_result$engaged, dat_result$est_contrast, type = "b", yaxt = "n", xaxt = "n", ylim = c(0,1), xlab = "Whether engaged in self-regulatory strategies on block j", ylab = "Prevalance of nicotine use on block j")
axis(1, at = c(0,1))
...
$$
\text{logit} Pr\left(\text{ANY NICOTINE USE}_{i} \left(t_{i,j+1} \right) = 1\right)= \beta_0 \left(t_{i,j}\right) + \beta_1 \left(t_{i,j}\right) \text{WHETHER ENGAGED} \left(t_{i,j}\right) + \text{ANY NICOTINE USE}_{i} \left(t_{i,j} \right)
$$
```{r}
dat_tvem <- dat_logreg
```
