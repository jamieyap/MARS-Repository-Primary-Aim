...
```{r}
dat_for_analytic_population_concurrent <- dat_for_analysis %>% select(-eligibility_lead1, -substance_is_any_nicotine_lead1) %>% mutate(is_cc = complete.cases(.))
...
```{r}
png(file = file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Associational-Nicotine-Use", "analytic_population_by_dp_concurrent.png"), width = 10, height = 10, units = "in", res = 600)
...
title(main = "Concurrent analysis at the momentary level: Nicotine Use")
...
```{r}
png(file = file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Associational-Nicotine-Use", "analytic_population_by_dp_lagged.png"), width = 10, height = 10, units = "in", res = 600)
...
title(main = "Lagged analysis at the momentary level: Nicotine Use")

dev.off()
```


# Association between engagement in self-regulatory strategies and nicotine use on current person-block

$$
\mathrm{logit} Pr \left( \text{ANY NICOTINE USE}_{i,j} = 1 \Big| I_{i, j} = 1 , \text{control covariates}, \text{WHETHER ENGAGED}_{i,j} \right) = \beta_0 + \text{c_age}\beta_1 + \text{c_is_male}\beta_2 + \text{c_is_latino}\beta_3 + \text{c_is_not_latino_and_black}\beta_4 + \text{c_is_not_latino_and_other}\beta_5 + \text{c_baseline_tobacco_history}\beta_6 + \text{c_has_partner}\beta_7 + \text{c_income_val}\beta_8 + \text{c_any_prompt}_{i,j}\beta_9 + \text{WHETHER ENGAGED}_{i,j} \beta_{10}
$$
...
```{r}
dat_logreg %>%
  mutate(any_nic = substance_is_any_nicotine) %>%
  group_by(any_nic) %>%
...
```{r}
fit <- geeglm(substance_is_any_nicotine ~ c_age + c_is_male + c_is_latino + c_is_not_latino_and_black + c_is_not_latino_and_other + c_baseline_tobacco_history + c_has_partner + c_income_val + c_any_prompt + Y, family = "binomial", data = dat_logreg, id = participant_id, waves = decision_point)
```
...
# Format output to 3 decimal places
dat_results_probability_scale %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "Exploratory-Analysis-Associational-Nicotine-Use", "block_level_probability_scale_concurrent.csv"), row.names = TRUE)
```
...
$$
\text{logit} Pr\left(\text{ANY NICOTINE USE}_{i} \left(t_{i,j} \right) = 1\right) = \boldsymbol{Z}_c\boldsymbol{\alpha} + \beta_0 \left(t_{i,j}\right) + \beta_1 \left(t_{i,j}\right) \text{c_ANY PROMPT} \left(t_{i,j}\right) + \beta_2 \left(t_{i,j}\right) \text{WHETHER ENGAGED} \left(t_{i,j}\right)
$$
...
for(current_num_knots in 1:3){
  model_tvem <- tvem(data=dat_tvem,
                     formula=substance_is_any_nicotine ~ c_any_prompt + Y,
                     id=participant_id,
                     time=decision_point,
...
# Association between engagement in self-regulatory strategies and nicotine use on next person-block

$$
\mathrm{logit} Pr \left( \text{ANY NICOTINE USE}_{i,j + 1} = 1 \Big| \left\{I_{i, j} = 1\right\} \cap \left\{I_{i, j+1} = 1\right\}, \text{control covariates}, \text{WHETHER ENGAGED}_{i,j} \right) = \beta_0 + \text{c_age}\beta_1 + \text{c_is_male}\beta_2 + \text{c_is_latino}\beta_3 + \text{c_is_not_latino_and_black}\beta_4 + \text{c_is_not_latino_and_other}\beta_5 + \text{c_baseline_tobacco_history}\beta_6 + \text{c_has_partner}\beta_7 + \text{c_income_val}\beta_8 + \text{c_any_prompt}_{i,j}\beta_9 + \text{ANY NICOTINE USE}_{i,j}\beta_{10} + \text{WHETHER ENGAGED}_{i,j} \beta_{11}
$$
...
```{r}
dat_logreg %>%
  mutate(any_nic_lead1 = substance_is_any_nicotine_lead1) %>%
...
```{r}
dat_tmp2 <- dat_logreg %>% 
  select(participant_id, decision_point, substance_is_any_nicotine, any_prompt)
```
...
dat_tmp2_summary <- dat_tmp2 %>%
  group_by(decision_point) %>%
  summarise(m_any_prompt = mean(any_prompt),
            m_substance_is_any_nicotine = mean(substance_is_any_nicotine))
```
...
dat_logreg <- dat_logreg %>%
  mutate(c_age = age - m_age,
         ...
         c_income_val = income_val - m_income_val) %>%
  mutate(c_any_prompt = any_prompt - m_any_prompt) %>%
  mutate(c_substance_is_any_nicotine = substance_is_any_nicotine - m_substance_is_any_nicotine)
```

```{r}
fit <- geeglm(substance_is_any_nicotine_lead1 ~ c_age + c_is_male + c_is_latino + c_is_not_latino_and_black + c_is_not_latino_and_other + c_baseline_tobacco_history + c_has_partner + c_income_val + c_any_prompt + c_substance_is_any_nicotine + Y, family = "binomial", data = dat_logreg, id = participant_id, waves = decision_point)
```
...
$$
\text{logit} Pr\left(\text{ANY NICOTINE USE}_{i} \left(t_{i,j+1} \right) = 1\right)= \boldsymbol{Z}_c\boldsymbol{\alpha} + \beta_0 \left(t_{i,j}\right) + \beta_1 \left(t_{i,j}\right) \text{c_ANY PROMPT}_{i} \left(t_{i,j} \right) + \beta_2 \left(t_{i,j}\right) \text{c_ANY NICOTINE USE}_{i} \left(t_{i,j} \right) + \beta_3 \left(t_{i,j}\right) \text{WHETHER ENGAGED} \left(t_{i,j}\right)
$$
...
for(current_num_knots in 1:3){
  model_tvem <- tvem(data=dat_tvem,
                     formula=substance_is_any_nicotine_lead1 ~ c_any_prompt + c_substance_is_any_nicotine + Y,
                     id=participant_id,
                     time=decision_point,
                     invar_effects=~ c_age + c_is_male + c_is_latino + c_is_not_latino_and_black + c_is_not_latino_and_other + c_baseline_tobacco_history + c_has_partner + c_income_val,
                     family = binomial(),
...
```{r}
if(file.exists("analysis-complete-case/formatted-output/Exploratory-Analysis-Associational-Nicotine-Use/Thumbs.db")){
  file.remove("analysis-complete-case/formatted-output/Exploratory-Analysis-Associational-Nicotine-Use/Thumbs.db")
}
```
