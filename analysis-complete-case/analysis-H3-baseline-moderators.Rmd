...
```{r}
dat_plot <- data.frame(val = vals_to_display, est = exp_estimates[3:83], rrLB95 = rrLB95[3:83], rrUB95 = rrUB95[3:83])

dat_plot

g <- ggplot(data = dat_plot, aes(x = val, y = est, ymin = rrLB95, ymax = rrUB95))
g <- g + geom_abline(slope = 0, intercept = 1, color = "red", linewidth = 1) + geom_ribbon(alpha = 0.3) + geom_line(linewidth = 2) + scale_y_continuous(limits = c(0.75,2.25), breaks = seq(0.75, 2.25, 0.25), name = "Risk Ratio and 95% CI's") + scale_x_continuous(name = "Value of Gratitude", breaks = c(3,4,5,6,7), labels = c(3,4,5,6,7))
...
```{r}
dat_plot <- data.frame(val = vals_to_display, est = exp_estimates[3:83], rrLB90 = rrLB90[3:83], rrUB90 = rrUB90[3:83])

dat_plot

g <- ggplot(data = dat_plot, aes(x = val, y = est, ymin = rrLB90, ymax = rrUB90))
g <- g + geom_abline(slope = 0, intercept = 1, color = "red", linewidth = 1) + geom_ribbon(alpha = 0.3) + geom_line(linewidth = 2) + scale_y_continuous(limits = c(0.75,2.25), breaks = seq(0.75, 2.25, 0.25), name = "Risk Ratio and 90% CI's") + scale_x_continuous(name = "Value of Gratitude", breaks = c(3,4,5,6,7), labels = c(3,4,5,6,7))
...
```{r}
dat_for_analysis_elig <- dat_for_analysis %>% filter(eligibility == 1)
dat_for_summary <- dat_for_analysis_elig %>% select(participant_id, gratitude) %>% unique(.)
dat_for_summary <- dat_for_summary %>% arrange(gratitude) %>% mutate(plot_id = 1:nrow(.))

summary(dat_for_summary$gratitude)

quantile(dat_for_summary$gratitude, probs = seq(0,1,0.01), na.rm = TRUE)
```

```{r}
g <- ggplot(data = dat_for_summary, aes(x = 0, y = gratitude))
g <- g + geom_boxplot(fill = "goldenrod", size = 1, width = 0.05) + scale_y_continuous(limits = c(1,7), breaks = seq(1,7,0.25), name = "Gratitude") + scale_x_continuous(limits = c(-0.2,0.2), name = "", breaks = c(10), labels = "") + geom_dotplot(binaxis='y', stackdir='center', alpha = 0.3, dotsize = 0.7)

g

ggsave(filename = file.path("analysis-complete-case", "formatted-output", "H3-baseline-moderators", "boxplot_H3_gratitude.png"), width = 7, height = 12, units = "in")
```

### Estimated effect on the log-risk ratio scale after centering gratitude

```{r}
dat_for_analysis_elig <- dat_for_analysis %>% filter(eligibility == 1)
dat_for_summary <- dat_for_analysis_elig %>% select(participant_id, gratitude) %>% unique(.)

mu <- mean(dat_for_summary$gratitude, na.rm = TRUE)
std_dev <- sd(dat_for_summary$gratitude, na.rm = TRUE)

out <- data.frame(mu = mu, std_dev = std_dev)

print(out)
```

```{r}
dat_for_analysis <- dat_for_analysis %>% mutate(gratitude_c = (gratitude - mu))
```
...
fit1 <- emee(
  ...
  treatment = "coinflip",
  rand_prob = 0.5,
  moderator_formula = ~ gratitude_c,  
  control_formula = ~ gratitude_c + age + is_male + is_latino + is_not_latino_and_black + is_not_latino_and_other + baseline_tobacco_history + has_partner + income_val + hour_coinflip_local + days_between_v1_and_coinflip_local + any_response_2qs + engagement_most_recent_eligible, 
  availability = "eligibility"
)
...
vals_to_display <- c(-2*std_dev, -1.5*std_dev, -std_dev, -0.5*std_dev, 0, 0.5*std_dev, std_dev, 1.5*std_dev, 2*std_dev)
...
# Format output to 3 decimal places
dat_result %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H3-baseline-moderators", "log_risk_ratio_scale_H3_gratitude_c.csv"), row.names = TRUE)
```

### Estimated effect on the risk ratio scale after centering gratitude
...
dat_result_exp_output %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H3-baseline-moderators", "risk_ratio_scale_H3_gratitude_c.csv"), row.names = TRUE)
```

```{r}
dat_plot <- data.frame(val = vals_to_display, est = exp_estimates[3:11], rrLB95 = rrLB95[3:11], rrUB95 = rrUB95[3:11])

dat_plot

g <- ggplot(data = dat_plot, aes(x = val, y = est, ymin = rrLB95, ymax = rrUB95))
g <- g + geom_abline(slope = 0, intercept = 1, color = "red", linewidth = 1) + geom_ribbon(alpha = 0.3) + geom_line(linewidth = 2) + scale_y_continuous(limits = c(0.75,2.25), breaks = seq(0.75, 2.25, 0.25), name = "Risk Ratio and 95% CI's") + scale_x_continuous(name = "Number of standard deviations above or below mean of gratitude", breaks = seq(-2,2,0.5), labels = seq(-2,2,0.5))

g

ggsave(filename = file.path("analysis-complete-case", "formatted-output", "H3-baseline-moderators", "risk_ratio_scale_H3_gratitude_c_CI95.png"), width = 7, height = 5, units = "in")
```

```{r}
dat_plot <- data.frame(val = vals_to_display, est = exp_estimates[3:11], rrLB90 = rrLB90[3:11], rrUB90 = rrUB90[3:11])

dat_plot

g <- ggplot(data = dat_plot, aes(x = val, y = est, ymin = rrLB90, ymax = rrUB90))
g <- g + geom_abline(slope = 0, intercept = 1, color = "red", linewidth = 1) + geom_ribbon(alpha = 0.3) + geom_line(linewidth = 2) + scale_y_continuous(limits = c(0.75,2.25), breaks = seq(0.75, 2.25, 0.25), name = "Risk Ratio and 90% CI's") + scale_x_continuous(name = "Number of standard deviations above or below mean of gratitude", breaks = seq(-2,2,0.5), labels = seq(-2,2,0.5))

g

ggsave(filename = file.path("analysis-complete-case", "formatted-output", "H3-baseline-moderators", "risk_ratio_scale_H3_gratitude_c_CI90.png"), width = 7, height = 5, units = "in")
```
