...
### Estimated effect on the log-risk ratio scale after standardizing gratitude
...
```{r}
dat_for_analysis <- dat_for_analysis %>% mutate(gratitude_cs = (gratitude - mu)/std_dev)
```
...
fit1 <- emee(
  ...
  treatment = "coinflip",
  rand_prob = 0.5,
  moderator_formula = ~ gratitude_cs,  
  control_formula = ~ gratitude_cs + age + is_male + is_latino + is_not_latino_and_black + is_not_latino_and_other + baseline_tobacco_history + has_partner + income_val + hour_coinflip_local + days_between_v1_and_coinflip_local + any_response_2qs + engagement_most_recent_eligible, 
  availability = "eligibility"
)
...
# Format output to 3 decimal places
dat_result %>% round(., 3) %>% format(., nsmall = 3) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H3-baseline-moderators", "log_risk_ratio_scale_H3_gratitude_cs.csv"), row.names = TRUE)
```

### Estimated effect on the risk ratio scale after standardizing gratitude
...
dat_result_exp_output %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "H3-baseline-moderators", "risk_ratio_scale_H3_gratitude_cs.csv"), row.names = TRUE)
```
...
ggsave(filename = file.path("analysis-complete-case", "formatted-output", "H3-baseline-moderators", "risk_ratio_scale_H3_gratitude_cs_CI95.png"), width = 7, height = 5, units = "in")
```
...
ggsave(filename = file.path("analysis-complete-case", "formatted-output", "H3-baseline-moderators", "risk_ratio_scale_H3_gratitude_cs_CI90.png"), width = 7, height = 5, units = "in")
```
