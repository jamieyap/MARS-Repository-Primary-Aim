...
results_obj <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", 1, "results_obj_secondary_hour_quadratic.rds"))
...
for(j in 1:num_terms){
  ...
  for(mi_dataset_num in 1:.__total_imputed_datasets){
    results_obj <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, "results_obj_secondary_hour_quadratic.rds"))
...
row.names(fit_pooled) <- c("High Effort Prompt (versus No Prompt)",
                           "High Effort Prompt (versus No Prompt) x Hour of Day",
                           "High Effort Prompt (versus No Prompt) x Square of Hour of Day",
                           "Low Effort Prompt (versus No Prompt)",
                           "Low Effort Prompt (versus No Prompt) x Hour of Day",
                           "Low Effort Prompt (versus No Prompt) x Square of Hour of Day",
                           "Indicator for High vs. Low", 
                           "Indicator for High vs. Low x Hour of Day",
                           "Indicator for High vs. Low x Square of Hour of Day",
                           paste("High vs. Low at Day ", 0:24, sep = ""))
...
for(mi_dataset_num in 1:.__total_imputed_datasets){
  ...
  for(idx_replicate in 1:.__par_total_replicates){
    results_obj_rep <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, paste("results_obj_secondary_hour_quadratic", "_replicate_", idx_replicate, ".rds", sep = "")))
...
row.names(dat_pbcom) <- c("High Effort Prompt (versus No Prompt)",
                          "High Effort Prompt (versus No Prompt) x Hour of Day",
                          "High Effort Prompt (versus No Prompt) x Square of Hour of Day",
                          "Low Effort Prompt (versus No Prompt)",
                          "Low Effort Prompt (versus No Prompt) x Hour of Day",
...
write.csv(fit_pooled_causal_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pooled_H2_causal_hour_quadratic.csv"), row.names = TRUE)
write.csv(fit_pooled_control_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pooled_H2_control_hour_quadratic.csv"), row.names = TRUE)
write.csv(dat_pbcom_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pbcom_H2_causal_hour_quadratic.csv"), row.names = TRUE)

###############################################################################
# Workflow: Plot
###############################################################################

png(file = file.path("analysis-multiple-imputation", "formatted-output", "log_risk_ratio_scale_secondary_hour_quadratic.png"), width = 6, height = 6, units = "in", res = 600)

plot(0:24, fit_pooled_causal[-c(1:9),]$Estimate, type = "o", ylim = c(-0.50, 0.70), xlab = "Hour of Day", ylab = "Treatment Effect on the Log-Scale")
lines(0:24, fit_pooled_causal[-c(1:9),]$LCL, type = "o", lty = 2)
lines(0:24, fit_pooled_causal[-c(1:9),]$UCL, type = "o", lty = 2)
abline(h = 0, lty = 2, col = "red")

dev.off()

png(file = file.path("analysis-multiple-imputation", "formatted-output", "risk_ratio_scale_secondary_hour_quadratic.png"), width = 6, height = 6, units = "in", res = 600)

plot(0:24, exp(fit_pooled_causal[-c(1:9),]$Estimate), type = "o", ylim = c(0.7,2), xlab = "Hour of Day", ylab = "Treatment Effect on the Risk Ratio-Scale")
lines(0:24, exp(fit_pooled_causal[-c(1:9),]$LCL), type = "o", lty = 2)
lines(0:24, exp(fit_pooled_causal[-c(1:9),]$UCL), type = "o", lty = 2)
