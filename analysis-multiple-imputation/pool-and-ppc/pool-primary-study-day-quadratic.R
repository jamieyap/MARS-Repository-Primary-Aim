...
row.names(fit_pooled) <- c("Treatment (Prompt = 1, No Prompt = 0)", "Treatment x Day", "Treatment x Square of Day", paste("Treatment Effect on Day ", 1:8, sep = ""))
fit_pooled_causal <- fit_pooled

dat_pool_stats <- bind_rows(list_pool_stats)
row.names(dat_pool_stats) <- c("Treatment (Prompt = 1, No Prompt = 0)", "Treatment x Day", "Treatment x Square of Day", paste("Treatment Effect on Day ", 1:8, sep = ""))
...
write.csv(fit_pooled_causal_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pooled_H1_causal_study_day_quadratic.csv"), row.names = TRUE)
write.csv(fit_pooled_control_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pooled_H1_control_study_day_quadratic.csv"), row.names = TRUE)
write.csv(dat_pbcom_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pbcom_H1_causal_study_day_quadratic.csv"), row.names = TRUE)
write.csv(dat_pool_stats_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pool_stats_H1_causal_study_day_quadratic.csv"), row.names = TRUE)

###############################################################################
# Workflow: Plot
###############################################################################

png(file = file.path("analysis-multiple-imputation", "formatted-output", "log_risk_ratio_scale_primary_study_day_quadratic.png"), width = 6, height = 6, units = "in", res = 600)

plot(1:8, fit_pooled_causal[-c(1:3),]$Estimate, type = "o", ylim = c(-0.10, 0.40), xlab = "Study Day", ylab = "Treatment Effect on the Log-Scale")
lines(1:8, fit_pooled_causal[-c(1:3),]$LCL, type = "o", lty = 2)
lines(1:8, fit_pooled_causal[-c(1:3),]$UCL, type = "o", lty = 2)
abline(h = 0, lty = 2, col = "red")

dev.off()

png(file = file.path("analysis-multiple-imputation", "formatted-output", "risk_ratio_scale_primary_study_day_quadratic.png"), width = 6, height = 6, units = "in", res = 600)

plot(1:8, exp(fit_pooled_causal[-c(1:3),]$Estimate), type = "o", ylim = c(0.7,1.5), xlab = "Study Day", ylab = "Treatment Effect on the Risk Ratio-Scale")
lines(1:8, exp(fit_pooled_causal[-c(1:3),]$LCL), type = "o", lty = 2)
lines(1:8, exp(fit_pooled_causal[-c(1:3),]$UCL), type = "o", lty = 2)
