...
results_obj <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", 1, "results_obj_secondary_hour_linear.rds"))
...
for(j in 1:num_terms){
  ...
  for(mi_dataset_num in 1:.__total_imputed_datasets){
    results_obj <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, "results_obj_secondary_hour_linear.rds"))
...
row.names(fit_pooled) <- c("High Effort Prompt (versus No Prompt)",
                           ...
                           "Indicator for High vs. Low", "Indicator for High vs. Low x Hour of Day",
...
for(mi_dataset_num in 1:.__total_imputed_datasets){
  ...
  for(idx_replicate in 1:.__par_total_replicates){
    results_obj_rep <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, paste("results_obj_secondary_hour_linear", "_replicate_", idx_replicate, ".rds", sep = "")))
...
write.csv(fit_pooled_causal_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pooled_H2_causal_hour_linear.csv"), row.names = TRUE)
write.csv(fit_pooled_control_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pooled_H2_control_hour_linear.csv"), row.names = TRUE)
write.csv(dat_pbcom_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pbcom_H2_causal_hour_linear.csv"), row.names = TRUE)

###############################################################################
# Workflow: Plot
###############################################################################

png(file = file.path("analysis-multiple-imputation", "formatted-output", "log_risk_ratio_scale_secondary_hour_linear.png"), width = 6, height = 6, units = "in", res = 600)

plot(0:24, fit_pooled_causal[-c(1:6),]$Estimate, type = "o", ylim = c(-0.50, 0.70), xlab = "Hour of Day", ylab = "Treatment Effect on the Log-Scale")
lines(0:24, fit_pooled_causal[-c(1:6),]$LCL, type = "o", lty = 2)
lines(0:24, fit_pooled_causal[-c(1:6),]$UCL, type = "o", lty = 2)
abline(h = 0, lty = 2, col = "red")

dev.off()

png(file = file.path("analysis-multiple-imputation", "formatted-output", "risk_ratio_scale_secondary_hour_linear.png"), width = 6, height = 6, units = "in", res = 600)

plot(0:24, exp(fit_pooled_causal[-c(1:6),]$Estimate), type = "o", ylim = c(0.7,2), xlab = "Hour of Day", ylab = "Treatment Effect on the Risk Ratio-Scale")
lines(0:24, exp(fit_pooled_causal[-c(1:6),]$LCL), type = "o", lty = 2)
lines(0:24, exp(fit_pooled_causal[-c(1:6),]$UCL), type = "o", lty = 2)
