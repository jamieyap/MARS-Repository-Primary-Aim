...
results_obj <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", 1, "sensitivity_results_obj_secondary_marginal.rds"))
...
for(j in 1:num_terms){
  ...
  for(mi_dataset_num in 1:.__total_imputed_datasets){
    results_obj <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, "sensitivity_results_obj_secondary_marginal.rds"))
...
for(mi_dataset_num in 1:.__total_imputed_datasets){
  ...
  for(idx_replicate in 1:.__par_total_replicates){
    results_obj_rep <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, paste("sensitivity_results_obj_secondary_marginal", "_replicate_", idx_replicate, ".rds", sep = "")))
...
write.csv(fit_pooled_causal_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "sensitivity_pooled_H2_causal.csv"), row.names = TRUE)
write.csv(fit_pooled_control_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "sensitivity_pooled_H2_control.csv"), row.names = TRUE)
write.csv(dat_pbcom_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "sensitivity_pbcom_H2_causal.csv"), row.names = TRUE)
write.csv(dat_pool_stats_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "sensitivity_pool_stats_H2_causal.csv"), row.names = TRUE)
