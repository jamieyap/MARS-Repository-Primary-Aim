...
row.names(fit_pooled) <- c("High Effort Prompt (versus No Prompt)",
                           ...
                           "High Effort Prompt (versus No Prompt) x Day in Study x Day in Study",
                           "Low Effort Prompt (versus No Prompt)",
                           "Low Effort Prompt (versus No Prompt) x Day in Study",
                           "Low Effort Prompt (versus No Prompt) x Day in Study x Day in Study",
                           "Indicator for High vs. Low", "Indicator for High vs. Low x Day in Study", "Indicator for High vs. Low x Square of Day in Study",
...
write.csv(fit_pooled_causal_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pooled_H2_causal_study_day_quadratic.csv"), row.names = TRUE)
write.csv(fit_pooled_control_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pooled_H2_control_study_day_quadratic.csv"), row.names = TRUE)
write.csv(dat_pbcom_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pbcom_H2_causal_study_day_quadratic.csv"), row.names = TRUE)
write.csv(dat_pool_stats_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pool_stats_H2_causal_study_day_quadratic.csv"), row.names = TRUE)

###############################################################################
# Workflow: Plot
###############################################################################

png(file = file.path("analysis-multiple-imputation", "formatted-output", "log_risk_ratio_scale_secondary_study_day_quadratic.png"), width = 6, height = 6, units = "in", res = 600)

plot(1:8, fit_pooled_causal[-c(1:9),]$Estimate, type = "o", ylim = c(-0.10, 0.40), xlab = "Study Day", ylab = "Treatment Effect on the Log-Scale")
lines(1:8, fit_pooled_causal[-c(1:9),]$LCL, type = "o", lty = 2)
lines(1:8, fit_pooled_causal[-c(1:9),]$UCL, type = "o", lty = 2)
abline(h = 0, lty = 2, col = "red")

dev.off()

png(file = file.path("analysis-multiple-imputation", "formatted-output", "risk_ratio_scale_secondary_study_day_quadratic.png"), width = 6, height = 6, units = "in", res = 600)

plot(1:8, exp(fit_pooled_causal[-c(1:9),]$Estimate), type = "o", ylim = c(0.7,1.5), xlab = "Study Day", ylab = "Treatment Effect on the Risk Ratio-Scale")
lines(1:8, exp(fit_pooled_causal[-c(1:9),]$LCL), type = "o", lty = 2)
lines(1:8, exp(fit_pooled_causal[-c(1:9),]$UCL), type = "o", lty = 2)
