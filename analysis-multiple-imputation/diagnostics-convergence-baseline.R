...
###############################################################################
imp <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "sequentially-completed-datasets", mi_dataset_num, "imp_obj_baseline.rds"))
...
png(filename = file.path(this_location, "baseline_convergence.png"), height = 20, width = 30, units = "in", res = 300)
plot(imp, layout = c(6,3))
dev.off()
