...
fit1 <- tryCatch(expr = {emee(
                 ...
                 treatment = "coinflip",
                 rand_prob = 0.5,
                 moderator_formula = ~ 1 + hour_coinflip_local + I(hour_coinflip_local * hour_coinflip_local),  
...
if(class(fit1) == "character"){
  ...
  Lmat <- matrix(c(rep(1,25), 0:24, (0:24)*(0:24)), ncol = 3, byrow = FALSE)
  results_obj <- summary(fit1, show_control_fit = TRUE, lincomb = Lmat)
}

saveRDS(fit1, file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, "fit_obj_primary_hour_quadratic.rds"))
saveRDS(results_obj, file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, "results_obj_primary_hour_quadratic.rds"))
...
for(idx_replicate in 1:max_replicate_id){
  ...
  saveRDS(fit1, file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, paste("fit_obj_primary_hour_quadratic", "_replicate_", idx_replicate, ".rds", sep = "")))
  saveRDS(results_obj, file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, paste("results_obj_primary_hour_quadratic", "_replicate_", idx_replicate, ".rds", sep = "")))
}
