...
dat_current_dp_outcome <- dat_timevarying_wide %>% 
  select(participant_id, Y_dp4, eligibility_dp4, coinflip_dp4, any_response_2qs_dp4, elig24hrs_dp4, matched_24hrs_dp4, any_recent_eligible_dp_dp4, engagement_most_recent_eligible_dp4, matched_recent_dp4,
...
dat_current_dp_outcome[["Y_sum_past24hrs_dp4"]] <- -1
dat_current_dp_outcome[["src_scored_mean_past24hrs_dp4"]] <- -1
dat_current_dp_outcome[["cigarette_counts_sum_past24hrs_dp4"]] <- -1

restriction_meet_string <- "eligibility_dp4 == 1 & elig24hrs_dp4 == 1"
restriction_violate_string <- "!(eligibility_dp4 == 1 & elig24hrs_dp4 == 1)"
...
for(i in 1:nrow(rows_meet_restriction)){
  current_matched <- rows_meet_restriction[["matched_24hrs_dp4"]][i]
  ...
  rows_meet_restriction[["Y_sum_past24hrs_dp4"]][i] <- rowSums(dat_eligibility * dat_Y)
  rows_meet_restriction[["src_scored_mean_past24hrs_dp4"]][i] <- rowSums(dat_eligibility * dat_src_scored)
  rows_meet_restriction[["cigarette_counts_sum_past24hrs_dp4"]][i] <- rowSums(dat_eligibility * dat_cigarette_counts)
}
...
for(i in 1:nrow(rows_meet_restriction)){
  ...
  passed_condition <- rows_meet_restriction[["any_recent_eligible_dp_dp4"]][i]
  
  if(passed_condition == 1){
    current_matched <- rows_meet_restriction[["matched_recent_dp4"]][i]
    ...
    rows_meet_restriction[["engagement_most_recent_eligible_dp4"]][i] <- dat_Y[[1]]
  }
}
...
# Note that at dp = 3, Y_sum_past24hrs_dp4 and engagement_most_recent_eligible_dp4 DO NOT have identical values. Hence, we only include both.
# Also note that there is no need to include any_recent_eligible_dp_dp4 since all rows who meet our past24 hour restriction will have a value of 1 for this variable.
my_list[["Y_dp4"]] <- as.formula(paste("Y_dp4 ~ coinflip_dp4 + any_response_2qs_dp4 + Y_sum_past24hrs_dp4 + engagement_most_recent_eligible_dp4 + src_scored_mean_past24hrs_dp4 + cigarette_counts_sum_past24hrs_dp4 + age + is_female + is_latino + is_not_latino_and_black + is_not_latino_and_other + baseline_tobacco_history + has_partner + income_val"))  
...
restriction_meet_string <- "eligibility_dp4 == 1 & elig24hrs_dp4 == 0"
restriction_violate_string <- "!(eligibility_dp4 == 1 & elig24hrs_dp4 == 0)"
...
indices_to_impute <- is.na(rows_meet_restriction[["Y_dp4"]])
rows_meet_restriction[["Y_dp4"]][indices_to_impute] <- rows_meet_restriction[["coinflip_dp4"]][indices_to_impute] * rbinom(n = sum(indices_to_impute), size = 1, prob = prob_when_prompt) + (1 - rows_meet_restriction[["coinflip_dp4"]][indices_to_impute]) * rbinom(n = sum(indices_to_impute), size = 1, prob = prob_when_none)
...
restriction_meet_string <- "eligibility_dp4 == 1"
restriction_violate_string <- "eligibility_dp4 == 0"
...
my_list <- list()
my_list[["src_scored_dp4"]] <- as.formula(paste("src_scored_dp4 ~ Y_dp4 + cigarette_counts_dp4 + srq_mean + mdes_neg_mean + gratitude + age + is_female + is_latino + is_not_latino_and_black + is_not_latino_and_other + baseline_tobacco_history + has_partner + income_val"))
my_list[["cigarette_counts_dp4"]] <- as.formula(paste("cigarette_counts_dp4 ~ Y_dp4 + src_scored_dp4 + srq_mean + mdes_neg_mean + gratitude + age + is_female + is_latino + is_not_latino_and_black + is_not_latino_and_other + baseline_tobacco_history + has_partner + income_val"))
