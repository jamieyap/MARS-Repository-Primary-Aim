...
keep_these_cols <- c("replicate_id", "participant_id", 
                     "is_high_effort", "is_low_effort",
                     "eligibility", "eligibility_lag1", "any_recent_eligible_dp",
                     "Y", "cigarette_counts", "src_scored", "cigarette_availability", "wearing_patch",
...
new_baseline_vars_to_consioder <- c("FinancialStrain", "nd_mean", "food_security_mean", "SSSladders", "pp1_1",
...
new_vars_to_consider <- c(new_time_varying_vars_to_consider, new_baseline_vars_to_consioder)
...
###############################################################################
# Workflow for wearing_patch
###############################################################################
LHS <- "wearing_patch"

consider_these_vars <- c(LHS, "is_high_effort", "is_low_effort",
...
# Save
###############################################################################
saveRDS(estimated_auc, file.path(path_multiple_imputation_pipeline_data, "sequentially-completed-datasets", mi_dataset_num, "estimated_auc.rds"))
